...
def solve_ks(u0, L, T_in, T_out, dt, nu, eval_sub):
    ...
    for idx, time in enumerate(np.arange(T_in, T_out, dt)):
        ...
        u_hat[:, idx + 1] = (1 / (1 - (dt / 2) * FL)) * (
            (1 + (dt / 2) * FL) * uhat_current
...
if __name__ == "__main__":
    ...
    save_path = f"tipping_KS_data_{ntraj}_traj_dt_{dt:.6g}.npy"

    nu0 = 0.073
    nu_scale = 0.0034
    nu_tau = 75.3
    nu = lambda t: (
        nu0 + nu_scale * np.exp(t / nu_tau)
    )  # tipping point occurs around t = 67 for initial condition given below
    ...
    for i in range(ntraj):
        ...
        # initial condition
        u0 = np.cos(x / 16) * (1 + np.sin(x / 16)) + np.random.uniform(
            -0.5, 0.5, x.shape
        )
        u_intermediate = solve_ks(u0, L, T0, T1, dt, nu, eval_sub)[
            :, -1
        ]  # getting to attractor
